 ...
 */
package com.example.opengl.arcore

import android.media.Image
import android.opengl.GLES30
import android.util.Log
import com.example.opengl.*
import com.example.opengl.Mesh.PrimitiveMode
...
class SpecularCubemapFilter constructor(
    ...
    private class ImportanceSampleCacheEntry constructor() {
        var direction: FloatArray = FloatArray(1)
        ...
        for (shader: Shader? in shaders) {
            for (i in importanceSampleCaches.indices) {
                ...
                for (j in cache.indices) {
                    val entry: ImportanceSampleCacheEntry? = cache!![j]
